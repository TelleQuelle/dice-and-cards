...
var ETH_GAS_RESERVE = new BigNumber(22e4);
var UINT16_MAX = 65535 | 0;
var ETH_MIN_AMOUNT = new BigNumber("100000000000000000");
...
var Ethereum = class extends Blockchain {
  ...
  rpcUrl;
  web3;
  minAmount = new BigNumber2(ETH_MIN_AMOUNT);
  ...
  async getPoolFee() {
    try {
      ...
      return new BigNumber2(result).div(1e4);
    } catch (error) {
  ...
  async stake(address, amount, source = "0") {
    ...
    if (new BigNumber2(amountWei).lt(this.minAmount)) {
  ...
  async unstake(address, amount, allowedInterchangeNum = 0, source = "0") {
    ...
    try {
      ...
      if (balance.lt(new BigNumber2(amount))) {
  ...
  async unstakePending(address, amount) {
    ...
    const bnAmount = new BigNumber2(amount);
  ...
  fromWeiToEther(amount) {
    return new BigNumber2(this.web3.utils.fromWei(amount, "ether"));
  }
  /**
  ...
  calculateGasLimit(gasConsumption) {
    return new BigNumber2(gasConsumption.toString()).plus(ETH_GAS_RESERVE).toNumber();
  }
  /**
...
var BigNumFromString = coerce(
  ...
  string(),
  (value) => {
    if (typeof value === "string") return new BigNumber3(value, 10);
...
var StakeAccount2 = class {
  ...
  stakeAccountState(currentEpoch) {
    ...
    const currentEpochBN = new BigNumber4(currentEpoch);
    const activationEpoch = new BigNumber4(stake.delegation.activationEpoch);
    const deactivationEpoch = new BigNumber4(stake.delegation.deactivationEpoch);
...
var Solana = class extends Blockchain {
  ...
  async unstake(sender, lamports, source) {
    try {
      ...
      let totalActiveStake = new BigNumber5(0);
      ...
      let lamportsBN = new BigNumber5(lamports);
      ...
      let i = 0;
      while (lamportsBN.gt(new BigNumber5(0)) && i < activeStakeAccounts.length) {
  ...
  async claim(sender) {
    try {
      ...
      const epochInfo = await this.connection.getEpochInfo();
      const tm = this.timestampInSec();
      let totalClaimableStake = new BigNumber5(0);
