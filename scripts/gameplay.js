...
function startLevel(levelNumber) {
    ...
    // Исправляем id: level-title отсутствует, используем level-number и level-name
    const levelNumberSpan = document.getElementById('level-number');
    const levelNameSpan = document.getElementById('level-name');
    if (levelNumberSpan && levelNameSpan) {
        levelNumberSpan.textContent = levelNumber;
        levelNameSpan.textContent = gameState.currentLevel.name;
    } else {
        console.error('level-number or level-name element not found');
    }

    const levelTarget = document.getElementById('target-score');
    if (levelTarget) levelTarget.textContent = gameState.currentLevel.target;
    else console.error('target-score element not found');

    const currentTurn = document.getElementById('current-turn');
    if (currentTurn) currentTurn.textContent = gameState.currentTurn;
    else console.error('current-turn element not found');

    const turnsLeft = document.getElementById('max-turns');
    if (turnsLeft) turnsLeft.textContent = gameState.turnsLeft;
    else console.error('max-turns element not found');
